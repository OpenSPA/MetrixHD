...
from . import _, initColorsConfig, initWeatherConfig, initOtherConfig, initFontsConfig, getTunerPositionList, appendSkinFile, \
    ...
    SKIN_OPENSPA_SOURCE, SKIN_OPENSPA_TARGET, SKIN_OPENSPA_TARGET_TMP, \
...
class ActivateSkinSettings:
	...
	def CheckSettings(self, onlyCheck=False):
		...
		if self.EHDenabled:
			self.service_name = 'enigma2-plugin-skins-metrix-spa-%s-icons' % self.EHDres.lower()
	...
	def applyChanges(self):
		...
		try:
			...
			skinSearchAndReplace.append(['skin_00o_openspa.xml', 'skin_00o_openspa.MySkin.xml'])
	...
	def iconFileCopy(self, target):
		...
		#plugin spaTeam
		spath = "/usr/share/enigma2/MetrixHD/%s/copy/Plugins/Extensions/spaTeam/icons/" % self.EHDres
		dpath = "/usr/lib/enigma2/python/Plugins/Extensions/spaTeam/icons/"
		self.FileCopy(target, spath, dpath)
